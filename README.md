...
| `arduino_neck_2ch.ino` | 목 부위의 초음파 센서를 제어하고 거리 데이터를 수집합니다. |
| `neck_cva_monitor_2ch.py` | 목 부위의 초음파 거리 데이터를 기반으로 전방 이동 및 자세 변화를 분석합니다. |
| `spine_curve_monitor_4ch_no_s4.py` | 등 부위의 초음파 거리 데이터를 기반으로 등 굽힘 및 자세 변화를 분석합니다. |
...
| 구분 | 파일 | 보드 | 주요 기능 | 
|---|---|---|---| 
| 압력센서 | `arduino_mega.ino` | Arduino Mega | 16-ELEMENT FSR-406 압력 센서 데이터 수집 | 
| 등/상체 센서 | `arduino_back.ino` | Arduino Uno | 5채널 HC-SR04 기반 등/상체 거리 측정 | 
| 목 센서 |`arduino_neck_2ch.ino` | Arduino Uno | 2채널 HC-SR04 기반 목 자세 측정 | 
| 시트 제어 | `smart_chair_arduino_3.ino` | Arduino Uno | 시트 및 등받이 Linear Actuator 제어 | 
| UI | `butten_code.ino` | Arduino Leonardo | START / PARK / END 버튼 및 LED 제어 | 
...
| **박정인** | Embedded / AI / Sensor / Seat Load Distribution | - FSR 압력센서 데이터 전처리 및 SFS 기반 최적 센서 위치 선정 알고리즘 구현<br>- SVM 기반 사용자 식별 및 5종 착석 자세 분류 모델 개발·검증<br>- Raspberry Pi 기반 실시간 압력 데이터 수집·전처리·추론 코드 구현 및 센서 통신 연동<br>- FSR 압력센서 배열을 고려한 시트 구조 설계 및 하중 전달·분산 구조 구현<br>- 실시간 사용자·자세 판정 로직 통합 및 임베디드 환경 동작 검증 |
...
| **장예은** | AI / Sensor / Circuit Design&Fabrication / Data Processing | - CNN 기반 사용자·자세 분류 모델을 구축 및 SVM 모델과 성능 비교·검증 <br>- FSR 센서 민감도 및 특성에 따른 회로 설계·실제 시트 내 기판 회로 제작<br>- Arduino Mega 기반 16채널 FSR 압력센서 데이터 통신 구조 구현<br>- 압력 센서 데이터 수집·통합·유사 데이터 제거·이상치 검토 과정 자동화 파이프라인 구축<br>- 초기 가상 데이터 구축 및 재학습용 데이터셋 구축<br>- 실제 데이터셋을 통한 자세 분류 모델 재학습·테스트를 통한 성능 검증 |
