...
Seat-ID는 센서 하드웨어, 임베디드 시스템, AI 모델, 사용자 인터페이스 및 시트 구동부를 결합하는 프로젝트로, 각 구성원이 담당 영역을 중심으로 개발을 진행하고 통합 단계에서는 공동으로 시스템을 검증하였다.

| 구성원 | 담당 Domain | 주요 업무 |
|---|---|---|
| **박정인** | **EMBEDDED · AI INFERENCE · SYSTEM INTEGRATION** | 압력센서 기반 실시간 데이터 처리 구조 설계, Raspberry Pi 기반 실시간 추론 코드 구현, SVM 기반 사용자 식별 및 자세 판정 로직 통합, 센서–임베디드 시스템 간 통신 연동, 모델 테스트 및 디버깅, 전체 시스템 통합 |
| **장예은** | **AI · SENSOR DATA · HARDWARE** | FSR 압력센서 기반 데이터 수집 구조 구성, Arduino Mega 기반 다채널 센서 통신 연동, 자세 분류 데이터셋 구축 및 전처리, 자세 분류 모델 학습 및 성능 검증, 센서 하드웨어 구성 |
| **팀 공동 수행** | **SYSTEM DESIGN · INTEGRATION · EVALUATION** | 시트 구조 설계, 센서 배치 검토, 시스템 기능 정의, 하드웨어 통합, 액추에이터 구동 검증, 시나리오 기반 테스트, 오류 분석, 보고서·발표·시연 준비 |

### Development Responsibility

프로젝트의 개발 과정은 크게 다음과 같은 영역으로 구분하였다.
...
Seat-ID는 개별 기능을 독립적으로 개발한 후 최종 단계에서 하나의 시스템으로 통합하였다.
...
Data Processing
      ...
      ├───────────────┐
      ▼               ▼
User Identification  Posture Classification
      │               │
      └───────┬───────┘
              ▼
       Raspberry Pi 5
      ...
      ┌───────┼──────────┐
      ▼       ▼          ▼
 Seat Setting  Warning   Driving Data
      │                    │
      ▼                    ▼
 Actuator              Report / UI
```

각 담당 영역에서 개발된 센서 데이터 처리, AI 모델, 실시간 추론 및 하드웨어 제어 기능을 연동하여 최종적으로 **사용자 식별 → 개인별 시트 설정 복원 → 자세 분석 → 상태 피드백 → 데이터 기록**으로 이어지는 통합 시스템을 구현하였다.
